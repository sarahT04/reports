...
async function getAllReportsFromCoach(req, res) {
  ...
  if (reportId !== undefined) {
    const firstData = await client.db('reports').collection('class_report').find({}).sort({ _id: -1 }).limit(1).toArray();
    if (stringToObjectId(reportId) === firstData[0]._id) {
      return res.status(201).json({ result: undefined });
    }
  }
  // Find the id of the last data
  resultDb = await client.db('reports').collection('class_report').aggregate([
    {
      $lookup: {
        from: 'classes',
        localField: 'kelas',
    ...
    { $unwind: '$coaches_data' },
    {
      $match: { '_id': { $lt: stringToObjectId(reportId) } }
    },
    {
      $project: {
        _id: 1,
