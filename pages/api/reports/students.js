...
async function getAllReportsFromCoach(req, res) {
  ...
  // Get body data
  const data = req.body;
  const { coach_id: coachId, report_id: reportId } = data;
  let resultDb;
  // Checking if we are at the end of the query or at the start of the query.
  if (reportId !== undefined) {
    const firstData = await client.db('reports')
    .collection('student_report')
    .find({ coach_id: stringToObjectId(coachId) })
    .sort({ _id: -1 })
    .limit(1).toArray();
    ...
    { $unwind: '$coaches_data' },
    {
      $match:
        reportId !== undefined
          ...
          : { 'coaches_data._id': stringToObjectId(coachId), '_id': { $lt: stringToObjectId(reportId) } }
    },
    {
      $project: {
        _id: 1,
